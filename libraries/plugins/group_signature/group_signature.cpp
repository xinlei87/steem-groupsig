...
namespace detail {
    ...
    void group_signature_plugin_impl::SharesGen(element_t *si, element_t *ti, element_t *Ei)
    {
    ...
    bool group_signature_plugin_impl:: VerifyShare(element_t Si, element_t Ti, element_t *Ei, int threshold, int i)
    {
        ...
        return element_cmp(result1, result2) == 0 ? true : false;
...
void group_signature_plugin::plugin_initialize(const appbase::variables_map& options)
{//初始化公钥
...
void group_signature_plugin::plugin_startup()
{
    ...
    G1_ELEMENT_ARRAY(ti, my->N, my->pairing);

    my->SharesGen(si, ti, Ei);//本节点的share

//-------节点间通信，互相发送秘钥分享

//----------将收到的share放入si 和ti中,接受时进行验证
    
    
    ZR_ELEMENT(alpha_i, my->pairing);
    ZR_ELEMENT(ri, my->pairing);
...
//只有一个节点：
    element_pow_zn(g_alpha_i[0],my->mpk->g, alpha_i);


    ZR_ELEMENT(I, my->pairing);
    ZR_ELEMENT(e, my->pairing);

    element_set1(I);
    element_set1(e);

    ZR_ELEMENT_ARRAY(Li, my->K, my->pairing);
    INIT_ARRAY_1(Li, my->K);
    ...
    G1_ELEMENT(g_alpha, my->pairing);
    element_set1(g_alpha);
    
    G1_ELEMENT(temp, my->pairing);
    for (int i = 0; i < my->K; i++)
    {
        element_pow_zn(temp, g_alpha_i[i], Li[i]);
        element_mul(g_alpha, g_alpha, temp);
    }
    element_set(my->mpk->h1, g_alpha);

    element_clear(temp);
    element_clear(g_alpha);

    my->GrpSetup(alpha_i, ri);
    
    CLEAR_ELEMENT(Li,my->N);
    CLEAR_ELEMENT(g_alpha_i, my->N);
    CLEAR_ELEMENT(si, my->N);
    CLEAR_ELEMENT(ti, my->N);
    CLEAR_ELEMENT(Ei, my->N);
